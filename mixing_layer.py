...
def run_live_simulation(kappa_mix, sigma_mix, V, T0):
    """Full pyrcel simulation -- the ground-truth path, and the only path
    until the surrogate is trained."""
...
def predict(kappa_mix, sigma_mix, V, T0):
    """Dispatch to the surrogate if trained, else run the real simulation.
 
    Returns a dict always containing 'method' ('surrogate' or 'live_simulation')
    and 'Nd'. The live_simulation path additionally returns 'model'/'out' for
    building trajectory/radius-profile plots; the surrogate path doesn't have
    those since it never ran a real simulation.
    """
